...
import static ru.netology.Test.Data.Registration.*;
...
public class UITest {
    ...
    void shouldHaveValidData() {
        User validUser = Data.Registration.generateValidUser();
    ...
    void shouldFakePassword() {
        User fakePassword = generateInvalidPassword("active");
        $("[data-test-id=login] input").setValue(fakePassword.getLogin());
        $("[data-test-id=password] input").setValue(fakePassword.getPassword());
        $("button[data-test-id=action-login]").click();
    ...
    void shouldFakeUsername() {
        User fakeLogin = generateInvalidLogin("active");
        $("[data-test-id=login] input").setValue(fakeLogin.getLogin());
        $("[data-test-id=password] input").setValue(fakeLogin.getPassword());
        $("button[data-test-id=action-login]").click();
        $("[data-test-id=error-notification] .notification__content").shouldHave(text("Неверно указан логин или пароль")).shouldBe(visible);
    }
}
